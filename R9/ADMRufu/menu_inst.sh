...
pid_inst(){
  ...
  proto="dropbear python stunnel4 v2ray $v_node badvpn ws-epro udp-custom psiphond squid openvpn dns-serve"
  portas=$(lsof -V -i -P -n | grep -v "ESTABLISHED" |grep -v "COMMAND")
  for list in $proto; do
    case $list in
      dropbear|python|stunnel4|v2ray|udp-custom|badvpn|squid|psiphond|ws-epro|openvpn|"$v_node") portas2=$(echo "$portas"|grep -w "LISTEN"|grep -w "$list") && [[ $(echo "${portas2}"|grep "$list") ]] && inst[$list]="\033[1;32m[ON] " || inst[$list]="\033[1;31m[OFF]";;
...
echo -ne "$(msg -verd "  [6]")$(msg -verm2 ">") $(msg -azu "BADVPN        ${inst[badvpn]}")" 
